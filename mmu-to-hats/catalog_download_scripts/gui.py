...
selected_rapper = st.sidebar.multiselect("🔍 Suche nach Rapper*innen", rapper_list, default=server_state.sel_rapper, label_visibility="visible")
with server_state_lock.sel_rapper:
...
selected_platforms = st.sidebar.multiselect("📺 Plattform wählen", platforms_list, default=server_state.sel_platforms)
with server_state_lock.sel_platforms:
...
onbeat_option = st.sidebar.selectbox(
    "🎵 Beat-Typ",
    beat_option_list,
    index=server_state.sel_beat_type
)
with server_state_lock.sel_beat_type:
...
written_option = st.sidebar.selectbox(
    "Match-Type",
    written_option_list,
    index=server_state.sel_match_type
)
with server_state_lock.sel_match_type:
...
min_clicks, max_clicks = server_state.db.execute("""SELECT MIN(video_clicks) AS min_clicks, MAX(video_clicks) AS max_clicks FROM battles;""").fetchone()
if not clicks_init:
    clicks_init = True
    with server_state_lock.clicks:
        server_state.clicks = (min_clicks, max_clicks)

min_clicks_selected, max_clicks_selected = st.sidebar.slider("Klicks", min_clicks, max_clicks, (min_clicks, max_clicks))
query += "AND b.video_clicks BETWEEN " + str(min_clicks_selected) + " AND " + str(max_clicks_selected) + "\n"
with server_state_lock.clicks:
    server_state.clicks = (min_clicks_selected, max_clicks_selected)

min_likes, max_likes = server_state.db.execute("""SELECT MIN(video_likes) AS min_likes, MAX(video_likes) AS max_likes FROM battles;""").fetchone()

if not likes_init:
    likes_init = True
    with server_state_lock.likes:
        server_state.likes = (min_likes, max_likes)
min_likes_selected, max_likes_selected = st.sidebar.slider("Likes", min_likes, max_likes, (min_likes, max_likes))
query += "AND b.video_likes BETWEEN " + str(min_likes_selected) + " AND " + str(max_likes_selected) + "\n"
with server_state_lock.likes:
    server_state.likes = (min_likes_selected, max_likes_selected)

min_comments, max_comments = server_state.db.execute("""SELECT MIN(video_comments) AS min_comments, MAX(video_comments) AS max_comments FROM battles;""").fetchone()
if not comments_Init:
    comments_Init = True
    with server_state_lock.comments:
        server_state.comments = (min_comments, max_comments)
min_comments_selected, max_comments_selected = st.sidebar.slider("Kommentare", min_comments, max_comments, (min_comments, max_comments))
query += "AND b.video_comments BETWEEN " + str(min_comments_selected) + " AND " + str(max_comments_selected) + "\n"
with server_state_lock.comments:
...
for rapper in selected_rapper:
    mask = (
            ~df["rapper_1"].str.contains(rapper, case=False, na=False) &
            df["rapper_2"].str.contains(rapper, case=False, na=False)
    )
    df.loc[mask, ["rapper_1", "rapper_2"]] = df.loc[mask, ["rapper_2", "rapper_1"]].values

st.dataframe(
    df,
    column_config={
        "video_title": "Video Name",
        "video_release": st.column_config.DatetimeColumn(label="Erscheinungdatum (Youtube)", format="DD.MM.YYYY"),
