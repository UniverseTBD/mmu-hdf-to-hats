...
class MaNGA(datasets.GeneratorBasedBuilder):
    ...
    def _info(cls):
        ...
        # not in original script, added to match correctly
        features["ra"] = Value("float64")
        features["dec"] = Value("float64")
        ...
        features["images"] = [
            {
                "filter": Value("string"),
                "flux": Array2D(
                    shape=(cls._image_size, cls._image_size), dtype="float32"
                ),
                "flux_units": Value("string"),
                "psf": Array2D(
                    shape=(cls._image_size, cls._image_size), dtype="float32"
        ...
        for file in itertools.chain.from_iterable(files):
            with h5py.File(file, "r") as data:
                ...
                for key in data.keys():
                    ...
                    example["spaxels"] = [
                        {
                            col_name: reshape_if_needed(col_name, i[col_idx])
                            for col_idx, col_name in enumerate(spax_cols)
                        }
